...
Vitals = (function () {
    ...
    Vitals.prototype.animate = function () {
        ...
        this.startAngle1 = this.drawVital($("#vitals").height() * 0.85, $("#vitals").height() * 0.05, 1, this.startAngle1, 0.005, "#00ff66", true, '.oxyegn', $("#vitals").height() * 0.07);
        this.startAngle2 = this.drawVital($("#vitals").height() * 0.70, $("#vitals").height() * 0.05, 1, this.startAngle2, 0.25, "#ffea00", false, '.energy', $("#vitals").height() * 0.23);
    };

    Vitals.prototype.drawVital = function (radius, width, direction, midValue, variance, color, strictlyDecrease, image_class_selector, image_height_offset) {
        if ($('body.moon').length > 0 && color == "#00ff66") {
        	midValue = this.drawCircle(radius, width, direction, 0, 0, variance, color, strictlyDecrease, false, image_class_selector, image_height_offset);
            midValue = this.drawCircle(radius, width, direction, 1.570796326797, 0, variance, color, strictlyDecrease, true, null, null);
        } else {
            midValue = this.drawCircle(radius, width, direction, midValue, 0, variance, color, strictlyDecrease, false, image_class_selector, image_height_offset);
    ...
    Vitals.prototype.drawCircle = function (radius, width, direction, startAngle, endAngle, variance, color, strictlyDecrease, invert, image_class_selector, image_height_offset) {
        ...
        this.context.save();
        if (image_class_selector) {
            imageObj = $(image_class_selector)[0]
...
$(document).ready(function () {
    ...
    if ($('body.stars').length > 0) {
        $('.zoomed-in-object img').width(bottomRightInfoWidthSize * 0.75);
        $('.zoomed-in-object img').css('position', 'absolute');
        $('.zoomed-in-object img').css('bottom', String(window.innerHeight * 0.25 + 10) + 'px');
        $('.zoomed-in-object img').css('left', bottomRightInfoWidthSize * 0.25);
